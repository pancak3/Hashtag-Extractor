...
string format_str(string key_str, unordered_map<string, string>
country_codes);
...
void combine_results(pair<unordered_map<string, unsigned long>,
    ...
    std::function<string(string)> printer = std::bind(format_str,
                                                      std::placeholders::_1,
                                                      country_codes);
    if (rank == 0) {
...
void easy_print(unordered_map<string, unsigned long> &map,
    ...
    for (int i = 0; pairs[i].second >= freq; i++) {
        std::cout << printer(pairs[i].first)
                  << " "
                  << printer(std::to_string(pairs[i].second))
                  //                  << printer(std::to_string(pairs[i].second))
                  << std::endl;
    }
