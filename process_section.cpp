...
void process_section(const char *filename, long long start, long long end,
    ...
    // 100 MB chunks for now, note that chunk num_worker cannot be less the shortest
    // line's length
    ...
    if (rank == 0) {
        ...
        while (working_worker - 1) {
            for (int i = 1; i < num_worker; i++) {
                ...
                MPI_Recv(&next_msg_len[i], 1, MPI_INT, i, 1,
...
#endif
        MPI_Send(&msg_len, 1, MPI_INT, 0, 1,
                 MPI_COMM_WORLD);
        MPI_Send(send_data.c_str(), msg_len, MPI_CHAR, 0, 0,
                 MPI_COMM_WORLD);
...
//#ifdef RESDEBUG
    if (rank == 0) {
        ...
        for (j = combined_lang_freq.begin();
             j != combined_lang_freq.end(); j++) {
            std::cout << j->first << " : " << j->second << std::endl;
            line_count += j->second;
        }
...
void send_combined_data(unordered_map<string, int> combined_res) {
    ...
    for (j = combined_res.begin();
        ...
        MPI_Send(&next_msg_len, 1, MPI_INT, 0, 1,
                 MPI_COMM_WORLD);
        MPI_Send(&send_data[0], next_msg_len, MPI_CHAR, 0, 0,
                 MPI_COMM_WORLD);
    }
}
